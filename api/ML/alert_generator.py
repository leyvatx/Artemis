...
Genera alertas automáticas basadas en predicciones ML.
Clasifica tipo y severidad, y crea payloads estructurados para el backend.
"""
...
class AlertGenerator:
    ...
    def generate_alert(self, 
        ...
        alert_payload = {
            ...
            # Metadatos ML
            'metadata': {
                'hr_zone': prediction_result.get('hr_zone', 'Unknown'),
                'hr_variability': prediction_result.get('metadata', {}).get('hr_variability', 0),
                'hr_elevated_sustained': prediction_result.get('metadata', {}).get('hr_elevated_sustained', 0),
                'hr_rapid_changes': prediction_result.get('metadata', {}).get('hr_rapid_changes', 0),
                'anomaly_score': prediction_result.get('metadata', {}).get('anomaly_score', 0),
                'ml_features': prediction_result.get('metadata', {}).get('features', {}),
                'sustained_pattern': prediction_result.get('metadata', {}).get('sustained_pattern', 'unknown'),
                'consecutive_high': prediction_result.get('metadata', {}).get('consecutive_high', 0),
                'consecutive_low': prediction_result.get('metadata', {}).get('consecutive_low', 0),
                'window_avg': prediction_result.get('metadata', {}).get('window_avg', 0),
                'trend': prediction_result.get('metadata', {}).get('trend', 'stable')
            }
        }
